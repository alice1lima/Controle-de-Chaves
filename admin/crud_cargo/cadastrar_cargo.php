...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($_POST['cargo'])) {
        echo "<script>alert('Favor preencher o nome do cargo.');</script>";
    } else {
        $cargo = $conn->real_escape_string($_POST['cargo']);

        // Verifica se o nome do cargo já está em uso
        $sql_check_cargo = "SELECT * FROM tb_cargos WHERE cargo = '$cargo'";
        $result_check_cargo = $conn->query($sql_check_cargo);

        if ($result_check_cargo->num_rows > 0) {
            echo "Este cargo já está cadastrado.";
        } else {
            // Insere os dados do cargo no banco de dados
            $sql_insert = "INSERT INTO tb_cargos (cargo) VALUES ('$cargo')";

            if ($conn->query($sql_insert)) {
                echo "<script>alert('Cadastro realizado com sucesso');</script>";
            } else {
                echo "Erro ao cadastrar cargo: " . $conn->error;
            }
        }
    }
}
...
<div class="header-container">
   ...
   <div id="popup" class="popup-container">
    ...
    <a href="visualizar_cargo.php">Visualizar Cargos</b></a>
   ...
   <div class="form-container">
        <form method="post" action="">
            <h3 class="exemplo"><b> Cadastrar Cargo</b></h3>
            <h4>Cargo:</h4>  
