...
// Verifica se o ID da ocupação foi passado na URL
if (isset($_GET["id_ocupacao"])) {
    $id_ocupacao = intval($_GET["id_ocupacao"]);

    // Consulta SQL para selecionar os dados da ocupação com o ID fornecido
    $sql = "SELECT * FROM tb_ocupacoes WHERE id_ocupacao = $id_ocupacao";
    $resultado = mysqli_query($conn, $sql);

    // Verifica se a ocupação foi encontrada
    if (mysqli_num_rows($resultado) == 1) {
        $ocupacao = mysqli_fetch_array($resultado);
        $id_usuario = $ocupacao['id_usuario'];
        $id_sala = $ocupacao['id_sala'];
        $dh_entrada = $ocupacao['dh_entrada'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id_usuario = $_SESSION['id_usuario']; // Mantendo o id_usuario do usuário logado
    $id_sala = $_POST["id_sala"];
    $observacoes = $_POST["observacoes"];

    // Atualiza os dados da ocupação no banco de dados
    $atualiza = mysqli_query($conn, "UPDATE tb_ocupacoes SET
        id_sala = '$id_sala', 
    ...
    if ($atualiza) {
        ...
        echo "Erro ao atualizar a ocupação: " . mysqli_error($conn);
    }
}
...
<div class="header-container">
   ...
   <div id="popup" class="popup-container">
    ...
    <a href="visualizar_ocupacao.php">Visualizar Chaves</a>
    <a href="editar_ocupacao.php">Editar Ocupação</a>
...
<form action="" method="post">
        ...
        <h4>Observações:  </h4>
            <textarea name="observacoes" style="width: 300px; height: 150px;"><?php echo htmlspecialchars($observacoes); ?></textarea>
       
        <p><input type="submit" value="Atualizar" class="button"></p>
        <p><a href="visualizar_ocupacao.php">Cancelar</a></p>
    </form>
    </div>
