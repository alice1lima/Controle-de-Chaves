...
if (isset($_GET["id_reserva"])) {
    ...
    // Consulta SQL para selecionar os dados da reserva com o ID fornecido e pertencente ao usuário logado
    $sql = "SELECT * FROM tb_reservas WHERE id_reserva = $id_reserva AND id_usuario = $id_usuario_sessao";
    ...
    if (mysqli_num_rows($resultado) == 1) {
        ...
        echo "Reserva não encontrada ou você não tem permissão para editá-la.";
        exit();
    }
} else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id_usuario = $id_usuario_sessao; // Usar o ID do usuário da sessão
    ...
    $atualiza = mysqli_query($conn, "UPDATE tb_reservas SET
        id_sala = '$id_sala', 
        entrada_previsao = '$entrada_previsao', 
        saida_previsao = '$saida_previsao'
        WHERE id_reserva = '$id_reserva' AND id_usuario = '$id_usuario_sessao'");
...
$resultado_salas = mysqli_query($conn, $sql_salas);
?>
...
<div class="header-container">
   ...
   <div id="popup" class="popup-container">
    ...
    <a href="visualizar_reservas.php">Visualizar Agendamento</a>
    <a href="editar_reservas.php">Editar Reserva</a>
...
<div class="form-container">
    <form action="" method="post">
        <input type="hidden" name="id_usuario" value="<?php echo $id_usuario_sessao; ?>">
        <h4>Sala:   </h4>
            <select name="id_sala">
                <?php
                while ($sala = mysqli_fetch_assoc($resultado_salas)) {
                    $selected = ($sala['id_sala'] == $id_sala) ? 'selected' : '';
                    echo "<option value='{$sala['id_sala']}' $selected>{$sala['sala']} - {$sala['bloco']}</option>";
                }
                ?>
            </select>
      
        <h4>Entrada Previsão:  </h4>
            <input type="datetime-local" name="entrada_previsao" value="<?php echo htmlspecialchars($entrada_previsao); ?>">
       
        <h4>Saída Previsão:   </h4>
            <input type="datetime-local" name="saida_previsao" value="<?php echo htmlspecialchars($saida_previsao); ?>">
      
        <p><input type="submit" value="Atualizar" class="button"></p>
        <p><a href="visualizar_reservas.php">Cancelar</a></p>
    </form>
    </div>
